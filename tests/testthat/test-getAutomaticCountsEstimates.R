...
test_that("getAutomaticCountsEstimates function throws error messages when input arguments are invalid", {
  ...
  expect_error(getAutomaticCountsEstimates(data.frame(area=c(500), red=c(-Inf)), 8, 4), "probeCounts cannot have any Inf or -Inf values")
  expect_error(getAutomaticCountsEstimates(data.frame(area=c(500), red=c(1.5)), 8, 4), "All counts in probeCounts must be non-negative integers")
  expect_error(getAutomaticCountsEstimates(data.frame(area=c(500), red=c(-1)), 8, 4), "All counts in probeCounts must be non-negative integers")
  expect_error(getAutomaticCountsEstimates(data.frame(area=c(500), red=c(-1.5)), 8, 4), "All counts in probeCounts must be non-negative integers")
