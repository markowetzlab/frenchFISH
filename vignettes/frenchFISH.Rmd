...
First, let's simulate a matrix with rows for nuclear blobs and columns for 
three different color probes (red, blue, and green). Entries in the matrix 
...
```{r readin_autocounts}
automatic_counts = cbind(area = round(runif(20,150,500), 2), 
...
our tissue section (which should be modified as appropriate depending on the
cell type and section height you are using). Note that before running the
Poisson model, we set the seed to insure our results will be reproducible:
...
Next, let's simulate a matrix of spot counts that have been manually 
