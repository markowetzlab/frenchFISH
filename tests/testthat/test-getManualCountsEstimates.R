...
test_that("getManualCountsEstimates function throws error messages when input arguments are invalid", {
  ...
  expect_error(getManualCountsEstimates(data.frame(red=c(-Inf)), 8, 4), "probeCounts cannot have any Inf or -Inf values")
  expect_error(getManualCountsEstimates(data.frame(red=c(1.5)), 8, 4), "All counts in probeCounts must be non-negative integers")
  expect_error(getManualCountsEstimates(data.frame(red=c(-1)), 8, 4), "All counts in probeCounts must be non-negative integers")
  expect_error(getManualCountsEstimates(data.frame(red=c(-1.5)), 8, 4), "All counts in probeCounts must be non-negative integers")
})
